...
describe("Mortgage Calculator", function() {
  ...
  describe("HTML", () => {
    ...
    it("should have the correct page title", () =>
      pageObject.evaluate(() => document.querySelector("body").innerText).then(text => {
        expect(text).to.contain("Mortgage Calculator");
      }));

    it('should have an input element with the name of "balance"', () =>
      pageObject.evaluate(() => document.querySelector("input[name=balance]")).then(input => expect(input).to.exist));

    it('should have an input element with the name of "rate"', () =>
      pageObject.evaluate(() => document.querySelector("input[name=rate]")).then(input => expect(input).to.exist));

    it('should have an input element with the name of "term"', () =>
      pageObject.evaluate(() => document.querySelector("select[name=term]")).then(input => expect(input).to.exist));

    it('should contain a button with the name of "submit"', () =>
      pageObject.evaluate(() => document.querySelector("button[name=submit]")).then(input => expect(input).to.exist));
  });
